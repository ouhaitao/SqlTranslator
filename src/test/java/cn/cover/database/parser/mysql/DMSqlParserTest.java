...
public class DMSqlParserTest {
  ...
  public static void main(String[] args) throws SqlTranslateException {
    ...
    String sql = "SELECT\n"
        ...
        + "\t\tti.hot_id \n"
        + "\tFROM\n"
        + "\t\tfm_news_hottestitem ti\n"
        + "\t\tLEFT JOIN fm_topic_topicdynamic td ON td.id = ti.relate_id \n"
        + "\tWHERE\n"
        + "\t\tti.`type` = 1 \n"
        + "\t\tAND td.STATUS = 4 \n"
        + "\t\tAND td.is_disable = 0 UNION ALL\n"
        + "\tSELECT\n"
        + "\t\tti.id,\n"
        + "\t\tti.`type`,\n"
        + "\t\tti.index_no,\n"
        ...
        + "\tFROM\n"
        + "\t\tfm_news_hottestitem ti\n"
        + "\t\tLEFT JOIN fm_news_brokenews bn ON bn.id = ti.relate_id \n"
        + "\tWHERE\n"
        + "\t\tti.`type` IN ( 2, 3 ) \n"
        + "\t\tAND bn.OPEN = 1 \n"
        + "\t\tAND bn.`SHOW` = 1 \n"
        ...
        + "\t\tid DESC \n"
        + "\t) hi\n"
        + "\tLEFT JOIN fm_news_hottest h ON h.`id` = hi.`hot_id` \n"
        + "WHERE\n"
        + "\th.`channel_id` =10\n"
        + "\tAND h.`status` = 1 \n"
        + "ORDER BY\n"
